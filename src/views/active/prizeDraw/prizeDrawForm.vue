    ...
    <div v-show="active === 0" >
      ...
      <el-form-item label="推广海报">
        <upload @fileChange="chooseImg" class="avatar-uploader" :width="width" :height="height" :style="imgStyle">
          <img v-if="prizeDrawForm.poster" :src="prizeDrawForm.poster" style="width: 100%;">
          <i v-else class="el-icon-plus"></i>
        </upload>
        <i v-if="prizeDrawForm.poster" class="el-icon-circle-close-outline" style="position: absolute; left: 180px; top: 6px; color: #fff;" @click="prizeDrawForm.poster=''"></i>
...
export default {
  ...
  methods: {
    chooseImg(data) {
      this.prizeDrawForm.poster = data
    },
    addModule() {
      this.openItem = true
      this.$nextTick(() => {
    ...
    init({ data }) {
      ...
      this.$http({
        ...
        this.prizeDrawForm.poster = data.data.poster ?  data.data.poster : ''
    ...
    setData(data) {
      this.isNew = false
      this.prizeDrawForm.totalNum = data.totalNum
      this.prizeDrawForm.lotteryType = data.lotteryType
      this.prizeDrawForm.lotteryStartTime = data.lotteryStartTime
      this.prizeDrawForm.expla = data.expla
      this.prizeDrawForm.wxNum = data.wxNum
      this.prizeDrawForm.poster = data.poster
      this.prizeDrawForm.activityType = 701
      this.prizeDrawForm.name = data.name
      this.prizeDrawForm.wxFans =  data.wxFans
      this.prizeDrawForm.integral = data.integral
      this.prizeDrawForm.showToC = data.showToC
      this.prizeDrawForm.integralNum = data.integralNum
      this.prizeDrawForm.fullText = data.fullText ? data.fullText : ''
      this.prizeDrawForm.time = []
      this.prizeDrawForm.time[0] = data.startTime
      this.prizeDrawForm.time[1] = data.endTime
      this.prizeDrawForm.useTime = []
      this.prizeDrawForm.useTime[0] = data.startUseTime
      this.prizeDrawForm.useTime[1] = data.endUseTime
      const attentionArr = []
      // 将注意事项转换成对应的值
      const attention = data.attention ? JSON.parse(data.attention) : []
      attention.forEach( element => {
        attentionArr.push({
          id: element.id,
          noteKey: element.noteKey,
      ...
      // 将风采展示转换成对应的值
      const photoShowArr = []
      const photoShow = data.photoShow ? JSON.parse(data.photoShow) : []
      photoShow.forEach( element => {
        photoShowArr.push({
          id: element.id,
          url: element.url
      ...
      this.prizeDrawForm.photoShow = JSON.stringify(photoShowArr)
      // 初始化奖品选项
      this.prizeDrawForm.prizeJson = data.prizeJson
      // 初始化选择
      this.getGroup().then(() => {
      ...
      this.$refs['prizeDrawForm'].validate((valid) => {
        ...
        if (valid) {
          ...
          if(this.prizeDrawForm.prizeJson == '[]') {
            ...
            this.ajaxData(submitData, url)
            if(this.isNew && this.saveData) {
              this.saveTemplate(submitData)
            }
          }
        } else {
          if(this.prizeDrawForm.time.length == 0) {
            this.$message({
              message:'您还未设置活动时间',
              type: 'info'
            })
          ...
          if(this.prizeDrawForm.useTime.length == 0) {
            this.$message({
              message:'您还未设置凭证使用时间',
              type: 'info'
            })
