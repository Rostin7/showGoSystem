<template>
  <div class="prizeDraw">
          ...
          <el-table :data="tableList" v-loading="listLoading" element-loading-text="拼命加载中">
            ...
            <el-table-column label="操作" align="center" header-align="center">
              <template slot-scope="scope">
                ...
                <router-link v-if="isAuth('/lotteryBy/joinList')" :to="'/prizeDrawOrderInfo/'+scope.row.activityId">
                  <el-button
                    size="mini"
                    type="text"
                    class="block block-center"
    ...
    <el-dialog title="添加促销" :visible.sync="addTypeChoose" :close-on-click-modal="false" :modal="true"  width="600px">
      <active-template :type="'prize'" :activeType="701" v-if="addTypeChoose" @addModel="addModel" @getModuleData="getModuleData" @close="addTypeChoose = !addTypeChoose"/>
    </el-dialog>
    <!-- 整机预览 -->
  ...
  methods: {
    ...
    getminiAppUrl(data) {
      ...
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ activityId: data.targetId, wxMiniUrlCode: data.code, activityType: 701 }, url, true)
      })
    },
    getWxAppEr(data) {
      const url = this.apiList.shop.prizeDraw.edit
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ activityId: data.targetId, wxMiniUrl: data.code, activityType: 701 }, url, true)
      })
    },
    // 点击添加
    addModel(index, data) {
