  ...
  <div class="enterName">
            ...
            <el-table-column label="操作" :width="200" align="center" header-align="center">
              <template slot-scope="scope">
                ...
                <el-button
                  ...
                  type="text"
                  class="block block-center"
                  v-if="isAuth('/enterName/mod') && (!scope.row.approvalId) && (!scope.row.showToC)"
    ...
    <el-dialog title="查看详情" :visible.sync="getDetail" :modal="true"  width="500px">
      ...
      <span slot="footer" class="dialog-footer">
       ...
       <enter-name v-if="getDetail" ref="enterName"/>
    </el-dialog>
    <!-- 整机预览 -->
...
import enterName from '../../preview/enterName'
export default {
  data() {
    return {
      getDetail: false,
  ...
  methods: {
    ...
    lookDetail(data) {
      ...
      this.getDetail = true
      this.$nextTick(() => {
        this.$refs.enterName.activity_id = data.activityId
        this.$refs.enterName.getDataList();
      })
    },
    getDataList() {
    ...
    changeState(index, data) {
      ...
      this.$http({
        ...
        method: 'post',
        data: this.$http.adornData({ showToC: state, activityId: data.activityId, activityType: 101 }, url, true)
      }).then(({ data }) => {
        if (data.result) {
