...
<script>
import formItem from '../prizeDraw/itemTable'
  ...
  methods: {
    ...
    init({ data }) {
      ...
      this.$http({
        ...
        this.prizeDrawForm.id = data.data.id
        ...
        const attentionArr = []
        // 将注意事项转换成对应的值
        const attention = data.data.attention ? JSON.parse(data.data.attention) : []
        ...
        // 将风采展示转换成对应的值
        const photoShowArr = []
        const photoShow = data.data.photoShow? JSON.parse(data.data.photoShow) : []
        ...
        // 初始化奖品选项
        this.prizeDrawForm.prizeJson = data.data.prizeJson
        // 初始化选择
