      ...
      <el-col :span="24">
        <el-card shadow="never">
          ...
          <el-form @submit.native.prevent :inline="true" class="demo-form-inline">
            <el-form-item style="margin-bottom: 0"><p class="main_header">活动商品添加</p></el-form-item>
            <el-form-item>
              <el-button @click.native="addTypeChoose = true" v-if="isAuth('/happyDay/addProduct')"><i class="el-icon-plus"></i></el-button>
            </el-form-item>
          </el-form>
          <el-table :data="dataList" v-loading="listLoading" element-loading-text="拼命加载中" ref="dragTable" >
            ...
            <el-table-column label="操作" align="center" header-align="center">
              <template slot-scope="scope">
                <el-button
                  v-if="isAuth('/happyDay/removeProduct')"
                  size="mini"
                  type="text"
...
export default {
  ...
  methods: {
    datadragEnd(newIndex, oldIndex) {
      ...
      const url = this.apiList.module.happyDayManage.sort
      const fromData = {
        groupId: this.typeId,
    ...
    async getDataList() {
      ...
      const url = this.apiList.module.happyDayManage.list
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ page: this.page, size: this.pageSize, happyDayId: this.id }, url, true)
      }).then(({ data }) => {
        if (data.result) {
          console.log(data)
    ...
    // 添加分组请求数据
    addActive(data) {
      const url = this.apiList.module.happyDayManage.add
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({happyDayId: data.happyDayId, activityId: data.activityId, activityType: data.activityType}, url, true)
      }).then(({ data }) =>{
        if(data.result) {
    ...
    submitDelete(index, data) {
      ...
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'danger' }
        ...
        const url = this.apiList.module.happyDayManage.del
        this.$http({
          url: this.$http.adornUrl(url),
          method: 'post',
          data: this.$http.adornData({ activityId: data.activityId, happyDayId:this.id, activityType: data.activityType }, url, true)
        }).then(({ data }) => {
          if (data.result) {
            // 删除成功
