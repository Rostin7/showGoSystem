      ...
      <el-col :span="24">
        <el-card shadow="never">
          ...
          <el-form @submit.native.prevent :inline="true" class="demo-form-inline">
            ...
            <el-form-item style="float: right" label="请输入兑换码">
              <el-input
                placeholder="请输入兑换码"
                v-model="searchName"
          ...
          <el-table :data="tableList" v-loading="listLoading" element-loading-text="拼命加载中" @filter-change="filterType">
            ...
            <el-table-column prop="codeKey" label="兑换码" align="center" header-align="center" >
              <template slot-scope="scope">
                {{ scope.row.codeKey ? scope.row.codeKey : '暂无兑换码数据' }}
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      const url = this.apiList.refund.listOutCoupon
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ page: this.page, size: this.pageSize, useState: this.useState, codeLike: this.idLike }, url , true)
      }).then(({ data }) => {
        if (data.result) {
