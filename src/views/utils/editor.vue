<template>
  <div class="editor">
    ...
    /> -->
    <div v-loading="loading" element-loading-text="网络图片资源转存中。。。。" element-loading-spinner="el-icon" :class="{'loadingBox': true, 'zIndex': loading}">
      <el-progress :percentage="percent" color="#409EFF"></el-progress>
    </div>
    <vue-neditor-wrap  v-model="content" :config="myConfig" :destroy="true" @ready="ready"></vue-neditor-wrap>
  </div>
</template>
<script>
...
export default {
  ...
  　　 return objE
    ...
    changeFile() {
      ...
      this.$customForeach(remoteImages, async (item, i) => {
        await this.uploadFile (item)
        this.percent = parseInt(i + 1 / remoteImages.length * 100)
      }, () => {
        this.loading = false
    ...
    uploadFile (fileUrl) {
      ...
      return this.$http({
        ...
        if (data.result) {
          ...
          if (this.str.indexOf(fileUrl) !== -1) {
            this.str = this.str.replace(fileUrl, data.data)
          } else if (this.str.indexOf(url_) !== -1){
            this.str = this.str.replace(url_, data.data)
          }
        }
      })
    },
