...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.$http({
        url: this.$http.adornUrl(this.apiList.shop.shopQualifiedShow.list),
        method: 'post',
        data: this.$http.adornData('', this.apiList.shop.shopQualifiedShow.list, true)
      }).then(({ data }) => {
        if (data.result) {
    ...
    handleRemove(file, fileList, id) {
      if (id) {
        this.$http({
          url: this.$http.adornUrl(this.apiList.shop.shopQualifiedShow.del),
          method: 'post',
          data: this.$http.adornData({ id: id }, this.apiList.shop.shopQualifiedShow.del, true)
        }).then(({ data }) => {
          if (data.result) {
        ...
        this.$http({
          url: this.$http.adornUrl(this.apiList.shop.shopQualifiedShow.del),
          method: 'post',
          data: this.$http.adornData({ id: file.id }, this.apiList.shop.shopQualifiedShow.del, true)
        }).then(({ data }) => {
          if (data.result) {
            console.log('删除成功')
          } else {
    ...
    addData(url) {
      this.$http({
        url: this.$http.adornUrl(this.apiList.shop.shopQualifiedShow.add),
        method: 'post',
        data: this.$http.adornData({ url: url }, this.apiList.shop.shopQualifiedShow.add, true)
      }).then(({ data }) => {
        if (data.result) {
          this.$message({
            type: 'success',
