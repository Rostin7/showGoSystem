<template>
  <div class="returnChild">
      ...
      <div class="right returnContainer">
        <ul>
          <li v-for="item in dataList" :key="item.id" class="lzl_single_post j_lzl_s_p first_no_border">
            ...
            <div class="lzl_cnt">
              <a class="at j_user_card" alog-group="p_author" target="_blank" >{{ item.name }}</a>:&nbsp;
              <span class="lzl_content_main" data-username="undefined">{{ item.content }}</span>
    ...
    <el-dialog title="提交" :visible.sync="dialogFormVisible" :modal="true" width="600px">
      <return-form :commentId="parentId" :superSmallCommentId="commentId" v-if="dialogFormVisible" @cancel="dialogFormVisible=false" @success="success"/>
    </el-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    // 获得回复详情
    getDataList() {
      const url = this.apiList.comment.return.detail
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ id: this.commentId }, url, true)
      }).then(({ data }) => {
        if (data.result) {
          this.avatar = data.data.avatar ? data.data.avatar : this.defaultImg
          this.userName = data.data.name
          this.content = data.data.content
          this.zanNum = data.data.zanNum
          this.createdTime = data.data.createdAt
          // this.parentId = this.$parent.$parent.$parent.$parent.commentId // 当前回复的回复id
        }
      })
    },
    ...
    // 获得子回复列表
    getChildList () {
      const url = this.apiList.comment.return.listChild
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ smallCommentId: this.commentId, page: this.page, size: this.pageSize }, url, true)
      }).then(({ data }) => {
        if (data.result) {
    ...
    delComment(data, self=false) {
      const url = this.apiList.comment.return.del
      this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'danger' }
      ).then(() => {
        this.$http({
          url: this.$http.adornUrl(url),
          method: 'post',
          data: this.$http.adornData({ smallCommentId: data }, url, true)
        }).then(({ data }) => {
          if (data.result) {
  ...
  created() {
    if (this.isAuth(this.apiList.comment.return.detail)) {
      this.getDataList()
    }
    if (this.isAuth(this.apiList.comment.return.listChild)) {
      this.getChildList()
    }
  }
}
