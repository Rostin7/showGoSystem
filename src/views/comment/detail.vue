<template>
  <div class="commentDetail">
    <el-card>
      ...
      <el-row :gutter="15">
        <el-col :span="18">
          ...
          <div class="return">
            <return-child v-for="item in dataList" :key="item.id" :parentId="commentId" :commentId="item.id" @delSelf="delSelf(item.id)"/>
          </div>
    ...
    <el-dialog title="提交" :visible.sync="dialogFormVisible" :modal="true" width="600px">
      <return-form :commentId="commentId" v-if="dialogFormVisible" @cancel="dialogFormVisible=false" @success="success"/>
    </el-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    init(id) {
      this.commentId = id
      const url = this.apiList.comment.detail
      this.$http({
        ...
        if (data.result) {
          ...
          this.dataDetail.smallNum = data.data.smallNum
        }
      })
    },
    getDataList() {
      const url = this.apiList.comment.return.list
       this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ commentId: this.commentId, page: this.page, size: this.pageSize }, url, true)
      }).then(({ data }) => {
  ...
  created() {
    ...
    if (this.isAuth(this.apiList.comment.return.list)) {
      this.getDataList()
    }
  }
}
