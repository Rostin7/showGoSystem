      ...
      <el-col :span="24">
        <el-card shadow="never">
          <el-form @submit.native.prevent :inline="true">
            ...
            <el-form-item style="float: right" v-if="isAuth('/approveActivity/search')">
              <el-button type="text" @click="search"><svg-icon icon-class="search"/></el-button>
            </el-form-item>
            <el-form-item style="float: right" label="请输入活动名称" v-if="isAuth('/approveActivity/search')">
              <el-input
                placeholder="请输入活动名称"
                v-model="searchName"
          ...
          <el-table :data="tableList" v-loading="listLoading" @filter-change="filterTag" element-loading-text="拼命加载中">
            <el-table-column prop="name" label="名称" align="center" header-align="center">
              <template slot-scope="scope">
                <preview :activeData="scope.row" :listId="scope.row.id" :url="apiList.showToCApproval.change.detail"/>
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <el-tag type="info" v-if="scope.row.approveStatus == 101">待审批</el-tag>
                <el-tag type="success" v-if="scope.row.approveStatus == 102">已通过</el-tag>
                <el-tag type="danger" v-if="scope.row.approveStatus == 103">已拒绝</el-tag>
              </template>
            </el-table-column>
            <el-table-column label="操作" align="center" header-align="center" v-if="isAuth('/showToCApproval/deal')">
              <template slot-scope="scope">
                <el-button
                  size="mini"
                  type="text"
                  v-if="scope.row.approveStatus == 101"
...
export default {
  ...
  methods: {
    // 筛选
    filterTag(data) {
      this.approveStatus = data.state[0]
      this.page = 1
      this.getDataList()
    },
    // 搜索
    search() {
      ...
      const url = this.apiList.showToCApproval.change.search
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ page: this.page, size: this.pageSize, content: this.searchName }, url, true)
      }).then(({ data }) => {
        if (data.result) {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
        data: this.$http.adornData({ page: this.page, size: this.pageSize, type: this.activityType, approveStatus:this.state }, url, true)
      }).then(({ data }) => {
        if (data.result) {
          this.tableList = data.data.records
          this.total = data.data.total
