  ...
  <el-form :model="commodityForm" @submit.native.prevent :rules="dataRule" :inline="false"  ref="commodityForm" label-width="110px" class="demo-ruleForm">
    ...
    <div v-show="active === 0" >
      ...
      <el-form-item label="主图">
        ...
        <div class="img_size_text" style="margin-top: -45px;">推荐尺寸: {{ 686 }} * {{ 320 }}</div>
      </el-form-item>
      <el-form-item label="封面">
        <upload @fileChange="chooseImg_imgUrlSquare" :width="123" :height="106" style="width: 123px;height: 106px;overflow: hidden;">
          <img v-if="commodityForm.imgUrlSquare" :src="commodityForm.imgUrlSquare" class="avatar">
          <i v-else class="el-icon-plus"></i>
        </upload>
        <div class="test_container">
        ...
        <div class="img_size_text">推荐尺寸: {{ 182 }} * {{ 158 }}</div>
      </el-form-item>
    ...
    init({ data }) {
      ...
      this.$http({
        ...
        if (data.result) {
          ...
          // }
          this.commodityForm.moduleGetPrice = data.data.moduleGetPrice > 0 ? data.data.moduleGetPrice / 100 : Math.round(data.data.price * Number(this.$cookie.get('moduleGetPrice')) * 100) / 10000
