  ...
  <div class="enterName">
    <el-card shadow="hover">
      <el-form :inline="true" @submit.native.prevent class="demo-form-inline">
        ...
        <el-form-item style="margin-bottom: 0;margin-left: 23px;">
          <p class="main_header">{{ activeName }}</p>
        </el-form-item>
        <el-form-item style="float: right">
          <refund v-if="isAuth('/order/moduleRefundOrder')" :only="false" :listData="multipleSelection" @changeData="getDataList"/>
        </el-form-item>
      </el-form>
      <el-table :data="tableList" v-loading="listLoading" element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      const url = this.apiList.shop.commodity.orderInfo
      this.$http({
        url: this.$http.adornUrl(url),
        method: 'post',
